...
@retry_on_failure()
def download_it(res, url, category, download_dir, thumbnail_url, info, checker=None):

    file_name = url.split('/')[-1]
    base_name = file_name.split('_%s.' % res)[0]
    base_path = os.path.join(download_dir, category)

    if os.path.exists(base_path) is False:
        os.mkdir(base_path)

    full_base_path = os.path.join(base_path, base_name)

    if os.path.exists(full_base_path) is False:
        os.mkdir(full_base_path)

    file_path = os.path.join(full_base_path, file_name)
    thumb_path = os.path.join(full_base_path, 'thumbnail.jpg')
    checker_path = os.path.join(full_base_path, 'checker.zip')
    ...
    if os.path.exists(file_path) is False:
        ...
        if info:
            ...
            metadata_path = os.path.join(full_base_path, 'info.json')
...
def main(download_dir, res='4k'):
    ...
    category_list = contents.find('ul', attrs={'id': 'category-list'})
    category_links = []

    # Get Categories
    for cat in category_list:
        href = cat.attrs['href'].split(' ')[0]

        if '?c=' in href:
            category_url = '%s%s' % (base_url, href)
    ...
    for link in category_links:
        ...
        for item in item_grid:
            ...
            # DO DOWNLOAD HERE
            download_it(res, full_href, category, download_dir, thumbnail_url, info, checker)
            # time.sleep(1)
